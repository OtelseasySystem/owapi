...
class QueryHandler {
    ...
    public function cancellationstatus($data,$agent_id) {
    ...
    public function xmlcancellationstatus($data,$agent_id) {
      $return = array();
      ...
      if ($query[0]['bookingFlg']==5 || $query[0]['bookingFlg']==3) {
        $inp_arr =[
            ...
            "Remarks"=>[
              "value" => 'Cancellation Checking'
            ]
        ];
        $xmlData =  $this->HotelCancel($inp_arr);

        if ($xmlData['Status']['StatusCode']=='01') {
          if (is_array($xmlData['CancellationCharge']) && is_array($xmlData['RefundAmount'])) {
            $return['RequestStatus'] = $xmlData['RequestStatus'];
            $return['Msg'] = 'Your request is in processing';
            return $return;
          } else {
            $return['CancellationCharge'] = is_array($xmlData['CancellationCharge']) ? 0 : $xmlData['CancellationCharge'];
            $return['RefundAmount'] = is_array($xmlData['RefundAmount']) ? 0 : $xmlData['RefundAmount'];
            $return['RequestStatus'] = $xmlData['RequestStatus'];
            return $return;
          }
